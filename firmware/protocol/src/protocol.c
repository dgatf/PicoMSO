                                  ...
                                  picomso_response_t *resp)
{
    ...
    if (in_len < PICOMSO_PACKET_HEADER_SIZE) {
        debug("\n[protocol] dispatch.rx invalid reason=packet_too_short in_len=%lu",
    ...
    if (hdr.magic != PICOMSO_PACKET_MAGIC) {
        debug("\n[protocol] dispatch.rx seq=%u msg=%s invalid reason=bad_magic magic=0x%04x",
              hdr.seq, msg_type_name((picomso_msg_type_t)hdr.msg_type), hdr.magic);
    ...
    if (hdr.version_major != PICOMSO_PROTOCOL_VERSION_MAJOR) {
        debug("\n[protocol] dispatch.rx seq=%u msg=%s invalid reason=bad_version version=%u.%u",
              hdr.seq, msg_type_name((picomso_msg_type_t)hdr.msg_type),
    ...
    if (hdr.length > PICOMSO_MAX_PAYLOAD_LEN) {
        debug("\n[protocol] dispatch.rx seq=%u msg=%s invalid reason=payload_too_large payload_len=%u",
              hdr.seq, msg_type_name((picomso_msg_type_t)hdr.msg_type), hdr.length);
    ...
    if (in_len < expected_total) {
        debug("\n[protocol] dispatch.rx seq=%u msg=%s invalid reason=truncated_payload payload_len=%u in_len=%lu expected=%lu",
              hdr.seq, msg_type_name((picomso_msg_type_t)hdr.msg_type), hdr.length,
              (unsigned long)in_len, (unsigned long)expected_total);
        picomso_write_error(hdr.seq, PICOMSO_STATUS_ERR_BAD_LEN,
    ...
    debug("\n[protocol] dispatch.rx seq=%u msg=%s payload_len=%u",
          hdr.seq, msg_type_name((picomso_msg_type_t)hdr.msg_type), hdr.length);
    ...
    case PICOMSO_MSG_GET_INFO:
    {
        ...
        debug("\n[protocol] dispatch.tx seq=%u msg=%s status=%s resp_used=%lu",
              hdr.seq, msg_type_name((picomso_msg_type_t)hdr.msg_type), status_name(status),
              (unsigned long)resp->used);
        return status;
    }
    ...
    default:
        debug("\n[protocol] dispatch.rx seq=%u msg=%u invalid reason=unknown_command",
              hdr.seq, hdr.msg_type);
