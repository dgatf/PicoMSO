 ...
 *
 * Returns one block of digital sample data via PICOMSO_MSG_DATA_BLOCK.
 *
 * Sample data is obtained from the module-level capture_buffer_t instance
 * (s_capture_buf) via capture_buffer_provider_get_block().  The buffer is
 * initialised lazily on the first request.  No real capture hardware (ADC,
 * PIO, DMA) is involved; the content is generated by capture_buffer_init()
 * and does not change between calls.  The block_id field increments with
 * each successful call so that the host can detect dropped or repeated blocks.
 *
...
/* Module-level capture buffer instance.  Lazy-initialised on first use. */
static capture_buffer_t s_capture_buf;
static uint8_t          s_capture_buf_inited = 0u;
                                                ...
                                                picomso_response_t            *resp)
{
    ...
    if (!s_capture_buf_inited) {
        capture_buffer_init(&s_capture_buf);
        s_capture_buf_inited = 1u;
    }
